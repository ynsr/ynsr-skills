# sample — Go tier skeleton (cobra + fang + go-pretty)

Single static binary CLI skeleton: wire real commands onto `main.go`'s shape. Runtime deps: none.

    ./install.sh                       # go build → ~/.local/bin/sample (+ cli-hub register)
    ./check.sh                         # golangci-lint + go test + verify-cli
    sample list [-o json|csv|tsv]      # sample rows; URL arg → fetch via retry helper (errors → exit 3)
    sample delete f.txt --dry-run      # plan → stdout; add --yes to execute (refuses = exit 2; never prompts)
