...
var (
	demoItems                     = []item{{"alpha", 3, "ok"}, {"beta", 5, "ok"}, {"gamma", 8, "stale"}}
	output                        string
	asJSON, quiet, dryRun, yes    bool
	exitCode                      int
	extraCmds                     []*cobra.Command // optional add-on commands; profile.go/service.go append in init()
)
...
var usagePrefixes = []string{"unknown command", "unknown flag:", "unknown shorthand flag:",
	"invalid argument", "flag needs an argument:", "required flag(s)", "accepts", "requires"}
...
func fetchItems(url, token string) ([]item, error) {
	...
	if token != "" {
		req.Header.Set("Authorization", "Bearer "+token) // secret: env/profile store only, never a flag
	}
...
// fetchCmdItems resolves the list source: URL arg, else the default profile (profiles build),
// else the builtin demo rows.
func fetchCmdItems(args []string) ([]item, error) {
	if len(args) == 1 {
...
Exit codes: 0 success, 1 error, 2 usage, 3 network, 4 partial. Stdout carries data only.`}
	...
	root.AddCommand(append([]*cobra.Command{
		&cobra.Command{Use: "list [URL]", Short: "Emit demo rows (URL: fetch via RetryClient)",
			Args: cobra.MaximumNArgs(1),
			RunE: func(_ *cobra.Command, args []string) error {
				items, err := fetchCmdItems(args)
				if err != nil {
